...
const API_URL = "https://cross-test-backend-server.onrender.com";
...
export default function App() {
  ...
  const generateMultipleArt = async (count: number) => {
    ...
    for (let i = 0; i < count; i++) {
      // Check if bulk generation was stopped
      if (stopBulkRef.current) {
        break;
      }
      
      // Get config with random values applied (for bulk generation)
      const config = getBulkConfig();
      
      // For bulk generation, palette is already a random array from getBulkConfig()
      const finalConfig = {
        ...config,
      ...
      try {
        ...
        // Add to queue and show immediately
        setArtQueue(prev => [...prev, newArt]);
        setCurrentArtIndex(prev => prev + 1);
        
        // Wait a bit before generating the next one (reduces server load and shows progress)
        if (i < count - 1 && !stopBulkRef.current) {
          await new Promise(resolve => setTimeout(resolve, 500)); // 500ms delay between generations
        }
